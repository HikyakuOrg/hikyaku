...
**API models are generated, not hand-written.** `pnpm gen:api` regenerates `lib/api/generated.ts` with `swagger-typescript-api`. It currently reads `../hikyaku-api/openapi.json` from a sibling checkout, because that spec is ahead of what is deployed; once it ships, switch the default back to `gen:api:remote` (the live https://api.hikyaku.org/api-docs-json). `generated.ts` is types-only and must never be edited. Shapes the spec does not describe go in `lib/api/manual.ts`; `lib/api/index.ts` re-exports both, so consumers just `import type { ... } from "@/lib/api"`.

**Never redeclare a request or response shape by hand.** Alias the generated DTO instead (`export type ConnectStatus = ConnectStatusDto`), which keeps the existing name for consumers while tracking the spec. Two exceptions, both deliberate: `lib/api/manual.ts` (`ListDriverDto` comes from a Supabase RPC, not the API) and `CreateInvitationInput` (a camelCase form input mapped onto the snake_case body). The ambient globals `Customer` and `Point` in `app/models/*` use inline `import(...)` types so they alias generated DTOs without becoming modules. Where a runtime list mirrors a generated union (pricing units, spending intervals), constrain it with `satisfies` or an exhaustive `Record` so backend drift fails the build.
