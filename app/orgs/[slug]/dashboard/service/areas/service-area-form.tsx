...
export type ServiceAreaPolygonFeature = GeoJSON.Feature<GeoJSON.Polygon, PolygonProperties>

export type ServiceAreaFormValues = {
    name: string
    polygon: ServiceAreaPolygonFeature
}
...
type ServiceAreaFormProps = {
    initialName?: string
    initialPolygon?: ServiceAreaPolygonFeature | null
    onSubmit: (values: ServiceAreaFormValues) => Promise<void>
    submitLabel: string
...
function normalizePolygonPrecision(feature: ServiceAreaPolygonFeature, decimalPlaces = 6): ServiceAreaPolygonFeature {
    return {
        ...feature,
        geometry: {
            ...feature.geometry,
...
function extractPolygonFeatures(input: GeoJSON.GeoJSON): ServiceAreaPolygonFeature[] {
    const polygonFeatures: ServiceAreaPolygonFeature[] = []
...
function getLargestPolygonFeature(features: ServiceAreaPolygonFeature[]) {
    return features.reduce<ServiceAreaPolygonFeature | null>((largest, feature) => {
        if (!largest) {
            return feature
        }
...
function isPolygonFeature(feature: GeoJSON.Feature): feature is ServiceAreaPolygonFeature {
...
function getPolygonBounds(feature: ServiceAreaPolygonFeature): LngLatBoundsLike {
...
export function ServiceAreaForm({
    ...
    useEffect(() => {
        ...
        const syncPolygonState = () => {
            ...
            const polygonFeatures = snapshot.filter(isPolygonFeature) as ServiceAreaPolygonFeature[]
    ...
    const processGeoJsonUpload = async (file: File, options?: { clearDrawnArea?: boolean }) => {
        ...
        try {
            ...
            if (uploadedPolygon) {
                ...
                const snapshot = drawRef.current.getSnapshot()
                const addedPolygon = ((snapshot.filter(isPolygonFeature) as ServiceAreaPolygonFeature[])
                    .find((feature) => feature.id === result.id) ?? uploadedPolygon) as ServiceAreaPolygonFeature
