...
// `buttonVariants` is a pure cva() function with no client-only dependencies, so
// it lives in its own module (no "use client"). This lets server components call
// it directly (e.g. `cn(buttonVariants())` on a styled <Link>) — calling it from
// the "use client" button.tsx module is not allowed under React Server Components.
